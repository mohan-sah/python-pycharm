...
class User(UserMixin, db.Model):
    __tablename__ = "users"
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    name: Mapped[str] = mapped_column(String(550), unique=True, nullable=False)
    password: Mapped[str] = mapped_column(String(550), nullable=False)
    email: Mapped[str] = mapped_column(String(550), unique=True, nullable=False)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user = User(
            name=form.name.data,
            password=hashed_and_salted_password(form.password.data),
        ...
        try:
            ...
            flash(message='Name already exist!')
